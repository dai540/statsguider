...
- Use `repeated = "yes"` when the same subject appears at three or more visits or conditions.
...
`guided_test()` uses the same logic as `select_test()`.
The difference is only the interface:

- `select_test()`: set the arguments directly
- `guided_test()`: answer the questions one by one
